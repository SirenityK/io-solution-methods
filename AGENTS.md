...
The current library includes these base algorithm families:
...
- `graphical-lp.ts` and `graphical-lp-plot.ts`: two-variable linear programming by the graphical method, constraint analysis, feasible-region construction, vertex/candidate evaluation, unbounded/infeasible/multiple-optimum status detection, and typed plot states.
- `nonlinear-graphical.ts` and `nonlinear-graphical-plot.ts`: two-variable nonlinear graphical programming with linear, axis, circle, ellipse, and quadratic constraints; candidate derivations; strict-boundary handling; tangent candidates; optimal/unattained/unbounded/infeasible status detection; and typed plot states.
- `matrix.ts` and shared types in `types.ts`: support utilities and shared domain types used by executable solution logic.
...
The current public method pages are:

- `/transporte`: transportation methods.
- `/simplex`: tabular simplex.
- `/grafico`: graphical method for two-variable linear programming.
- `/no-lineal`: graphical method for two-variable nonlinear programming.

## Math notation and KaTeX rendering

Use KaTeX for mathematical notation shown in the frontend, especially formulas, inequalities, ratios, objective functions, row operations, symbolic transformations, constraint boundaries, tangent equations, candidate points, and exact values used in simplex, transportation, graphical linear programming, or nonlinear graphical explanations.
...
The project ships a self-contained HTML manual generated by `scripts/build-manual.mjs`. The manual explains, in Spanish (MX), how to use the site and how each algorithm works mathematically. It is intended to be printed as a PDF for the course teacher.

- Run `bun scripts/build-manual.mjs` to regenerate `manual.html`.
- When a **new method page** is added to the site, update the build script so the manual covers it: add the corresponding usage section (Chapter 2) and the mathematical explanation section (Chapter 3), following the existing structure and tone.
- When an **existing algorithm is modified** in `src/lib` (e.g., its mathematical behavior, statuses, or trace output changes), update the relevant section in Chapter 3 of the build script so the manual stays accurate.
- The manual content must remain in **Spanish (MX)** with correct accentuation and proper KaTeX for formulas.
- The manual must **never mention AI tools, the AGENTS.md file, or internal agent configuration**. It is a user-facing document for the professor and students. Not development-facing.
